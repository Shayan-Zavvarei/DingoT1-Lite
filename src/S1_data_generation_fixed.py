...
def generate_detector_data(n_samples=100, output_file='./data/detector_strain_full.h5'):
    ...
    for i in tqdm(range(n_samples)):
        ...
        try:
            ...
            signal_L1 = det_L1.project_wave(hp, hc, ra, dec, psi)
            signal_V1 = det_V1.project_wave(hp, hc, ra, dec, psi)
            
            signal_H1.resize(target_len)
            signal_L1.resize(target_len)
            signal_V1.resize(target_len)
            ...
            noise_L1.resize(target_len)
            noise_V1.resize(target_len)
            
            strain_H1 = signal_H1.numpy() + noise_H1.numpy()
            strain_L1 = signal_L1.numpy() + noise_L1.numpy()
            strain_V1 = signal_V1.numpy() + noise_V1.numpy()
            
            all_hp.append(np.array(hp))
            all_hc.append(np.array(hc))
            all_signal_H1.append(np.array(signal_H1))
            all_signal_L1.append(np.array(signal_L1))
            all_signal_V1.append(np.array(signal_V1))
            all_noise_H1.append(np.array(noise_H1))
            all_noise_L1.append(np.array(noise_L1))
            all_noise_V1.append(np.array(noise_V1))
